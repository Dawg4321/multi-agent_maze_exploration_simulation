...
void RobotMaster_C::move2CellRequest(Message* request){
    ...
    if(robot_causing_collision == NULL){ // if no robot was found to be causing a collision
        ...
        if(robot_causing_collision->robot_position == target_cell && (robot_causing_collision->next_cell == NULL && robot_causing_collision->awaiting_next_cell == false || *robot_causing_collision->next_cell == current_robot_info->robot_position)){
            
            delete response_data; // deleting response data as it is not needed
            ...
            // clearing next and target cell from robots as new values will be generated by new requests
            current_robot_info->next_cell = NULL;
            robot_causing_collision->next_cell = NULL;
            current_robot_info->robot_target = NULL;
            robot_causing_collision->robot_target = NULL;
