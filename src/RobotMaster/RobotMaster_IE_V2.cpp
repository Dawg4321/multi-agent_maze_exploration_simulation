...
RobotMaster_IE_V2::RobotMaster_IE_V2(){
...
/*
void RobotMaster_IE_V2::reserveCellRequest(Message* request){
    ...
    else if(robot_info->planned_path.size() == 1 && reserving_robot->planned_path.size() > 1){ // if the target cell has already been reserved and the reserving robot is closer to the target
        // reserving target cell + updating current planned path
        robot_info->planned_path = request_data->planned_path; // setting planned path to robot's target
        robot_info->robot_target = target_cell; // gathering robot's target cell 
...
void RobotMaster_IE_V2::handleAlreadyReservedCell(RobotInfo* current_robot, RobotInfo* reserving_robot, m_reserveCellRequest* request_data, m_reserveCellResponse* response_data, Coordinates target_cell){
